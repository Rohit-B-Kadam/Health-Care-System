...
<%
	...
	try
	{
		...
		sql = "select * from Diagnostic_Labs where lab_id = "+appointedId+";";	
...
<tr>
 		<td>Select the test</td>
	         <td>   <select name="test_selected" size="5" multiple>
<%
			appointedId = (String) session.getAttribute("appointedId");
			try
			{
				Class.forName("org.postgresql.Driver");
      	      	  		con=DriverManager.getConnection("jdbc:postgresql://192.168.100.253/tydb37","ty37","");
      	          		st=con.createStatement();
				rs=st.executeQuery("select * from Tests where test_id in (select test_id from test_provider where lab_id = '"+appointedId+"' );");
				int i = 1;
				while( rs.next() )
				{
			%>	
...
if(request.getMethod().equalsIgnoreCase("POST"))
{
	...
	String[] testSelected = request.getParameterValues("test_selected");	
	...
	try
	{
		...
		con=DriverManager.getConnection("jdbc:postgresql://192.168.100.253/tydb37","ty37","");
		st = con.createStatement();
		sql = "select count(*) from Diagnostic_Tests;";	
		rs = st.executeQuery(sql);
		rs.next();
		int count = Integer.parseInt(rs.getString(1));
		count++;
		sql = "select * from Diagnostic_Tests where appointment_date = '"+date+"' and  appointment_time = '"+time+"' and lab_id  = '"+appointedId+"';";
		rs = st.executeQuery(sql);
		
		if(rs.next())
		{
			out.print("<br>time is already book");
		}
		else if(testSelected == null)
		{
			out.print("<br>no test is selected");
		}		
		else
		{
			sql = "insert into Diagnostic_Tests values( '"+count+"','"+date+"','"+time+"','','"+patientId+"','"+appointedId+"');";			
			st.execute(sql);
			
			sql = "select count(*) from Test_Selected;";	
			rs = st.executeQuery(sql);
			rs.next();
			int countTest = Integer.parseInt(rs.getString(1));
			countTest++;
				
					for(String t : testSelected)
					{
						sql = "insert into Test_Selected values('"+countTest+"','"+t+"','"+count+"');";					
						st.execute(sql);
						countTest++;
					}			
